...
const getBinRuntime = (json, contract, alias) => {
    let bin_runtime = undefined

    // First priority: bin_runtime specified in the configuration file.
    bin_runtime = json.implementations[alias].bin_runtime
    if (bin_runtime) return bin_runtime

    // Second priority: bin_runtime from the compiled output (useless for contracts with immutables).
    bin_runtime = contract["evm"]["deployedBytecode"]["object"]
    return bin_runtime
}
...
module.exports = (json) => {
  ...
  const contracts = {};
  const sourceList = Object.keys(dapp.sources)
  Object.keys(json.implementations)
    .forEach(alias => {
      ...
      contract = dapp.contracts[src][name]
      if (!contract) {
          revert(`No contract named ${name} found for file ${src} in ${dappout}`)
      }
      ...
      const bin_runtime = getBinRuntime(json, contract, alias)
      const srcmap_runtime = contract["evm"]["bytecode"]["sourceMap"]
      const srcmap = contract["evm"]["deployedBytecode"]["sourceMap"]
